defmodule Symphony.WorkspaceAndConfigTest do
  ...
  alias Symphony.Config.Schema.{Codex, StringOrMap}
  ...
  test "workspace replaces stale non-directory paths" do
    ...
    try do
      ...
      assert {:ok, canonical_workspace} = Symphony.PathSafety.canonicalize(stale_workspace)
  ...
  test "workspace rejects symlink escapes under the configured root" do
    ...
    try do
      ...
      assert {:ok, canonical_outside_root} = Symphony.PathSafety.canonicalize(outside_root)
      assert {:ok, canonical_workspace_root} = Symphony.PathSafety.canonicalize(workspace_root)
  ...
  test "workspace canonicalizes symlinked workspace roots before creating issue directories" do
    ...
    try do
      ...
      assert {:ok, canonical_workspace} =
               Symphony.PathSafety.canonicalize(Path.join(actual_root, "MT-LINK"))
  ...
  test "workspace remove rejects the workspace root itself with a distinct error" do
    ...
    try do
      ...
      assert {:ok, canonical_workspace_root} =
               Symphony.PathSafety.canonicalize(workspace_root)
  ...
  test "config reads defaults for optional settings" do
    ...
    assert {:ok, canonical_default_workspace_root} =
             Symphony.PathSafety.canonicalize(Path.join(System.tmp_dir!(), "symphony_workspaces"))
  ...
  test "path safety returns errors for invalid path segments" do
    ...
    assert {:error, {:path_canonicalize_failed, ^expanded_path, :enametoolong}} =
             Symphony.PathSafety.canonicalize(path)
  end
