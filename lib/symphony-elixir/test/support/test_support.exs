defmodule Symphony.TestSupport do
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      alias Symphony.PromptRenderer, as: PromptBuilder
      ...
      setup do
        ...
        if Process.whereis(Symphony.WorkflowStore), do: Symphony.WorkflowStore.force_reload()
        stop_default_http_server()

        on_exit(fn ->
          Application.delete_env(:symphony, :workflow_file_path)
          Application.delete_env(:symphony, :server_port_override)
          Application.delete_env(:symphony, :memory_tracker_issues)
          Application.delete_env(:symphony, :memory_tracker_recipient)
  ...
  def write_workflow_file!(path, overrides \\ []) do
    ...
    if Process.whereis(Symphony.WorkflowStore) do
      try do
        Symphony.WorkflowStore.force_reload()
      catch
        :exit, _reason -> :ok
      end
    end
  ...
  def stop_default_http_server do
    if Process.whereis(Symphony.Supervisor) do
  ...
  defp stop_default_http_server_child do
    case find_http_server_child() do
      {Symphony.HttpServer, pid, _type, _modules} when is_pid(pid) ->
        terminate_http_server(pid)
  ...
  defp find_http_server_child do
    Enum.find(Supervisor.which_children(Symphony.Supervisor), &http_server_child?/1)
  end

  defp http_server_child?({Symphony.HttpServer, _pid, _type, _modules}), do: true
  defp http_server_child?(_child), do: false

  defp terminate_http_server(pid) do
    :ok = Supervisor.terminate_child(Symphony.Supervisor, Symphony.HttpServer)
