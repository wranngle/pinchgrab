defmodule Symphony.StatusDashboard do
  ...
  alias Symphony.{Config, HttpServer}
  ...
  # ========== Symphony extensions (not in upstream) ==========
  # Per spec § 13.6: humanized event summaries are dashboard-only and
  # never depended on by the orchestrator. The two helpers below are
  # consumed by `Symphony.Web.Live.DashboardLive` and the
  # `/api/snapshot` controller for the "recent activity" pane.
  ...
  def recent_events(%{running: running}, opts) when is_list(running) do
    ...
    running
    |> Enum.flat_map(&recent_event_from_running/1)
  ...
  defp recent_event_from_running(entry) do
      ...
      event ->
        ...
        [
          %{
            ...
            at: format_iso8601_extension(ts),
            at_sort: sort_key_extension(ts),
  ...
  defp format_iso8601_extension(%DateTime{} = dt) do
    dt |> DateTime.truncate(:second) |> DateTime.to_iso8601()
  end

  defp format_iso8601_extension(_), do: nil

  defp sort_key_extension(%DateTime{} = dt), do: DateTime.to_unix(dt, :millisecond)
  defp sort_key_extension(_), do: 0
end
