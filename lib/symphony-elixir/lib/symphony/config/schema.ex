defmodule Symphony.Config.Schema do
  @moduledoc false
  ...
  defmodule Tracker do
    ...
    embedded_schema do
      ...
      # Destination state for clean agent exit. When set, the orchestrator
      # transitions the issue (via `Tracker.update_issue_state/2`) on
      # `:normal` worker exit instead of scheduling a `:continuation`
      # retry. Required for any tracker without a native state API to
      # reach a terminal state — without it, LocalMarkdown items spin in
      # `:continuation` forever (spec § 8.4).
      field(:success_state, :string)
      # Destination state for retry exhaustion (after `failure_max_attempts`
      # `:failure` retries). When set, the orchestrator transitions the
      # issue and stops re-scheduling the worker.
      field(:failure_state, :string)
      field(:failure_max_attempts, :integer, default: 3)
      # Polling cadence for `Symphony.RetryPromoter`. Each tick scans
      # `<issues_root>/<failure_state>/` for items whose front matter
      # carries an `auto_retry_budget` greater than zero, decrements the
      # budget, and moves the file back to the first `active_states`
      # directory so the orchestrator picks it up on the next poll. A
      # bounded self-correction loop on top of the terminal failure
      # transition (spec § 8.4 / § 11.1).
      field(:failure_promotion_interval_ms, :integer, default: 1_800_000)
    end
    ...
    def changeset(schema, attrs) do
      ...
      |> cast(
        ...
        [
          ...
          :failure_promotion_interval_ms
        ],
        empty_values: []
      )
      |> validate_number(:failure_max_attempts, greater_than: 0)
      |> validate_number(:failure_promotion_interval_ms, greater_than: 0)
    end
  end
