// Subset of lucide.dev icons inlined as SVG inner-markup.
// Each entry is the body of <svg ... > ... </svg>; svgString() wraps it.
// Sizes default to 16; override with the size argument.
//
// MIT — https://github.com/lucide-icons/lucide
...
const ICONS: Record<string, string> = {
  ...
  // Stylised "pinch" — two opposing curves meeting at a center dot.
  ...
  // Tree-of-rows — used for "Split group" action (denotes one node fanning
  // out into siblings). Lucide's `list-tree`.
  'list-tree': '<path d="M21 12h-8"/><path d="M21 6H8"/><path d="M21 18h-8"/><path d="M3 6v4c0 1.1.9 2 2 2h3"/><path d="M3 10v6c0 1.1.9 2 2 2h3"/>',
  // Generic split icon as a fallback option.
  ...
  // Cardboard-style box used for "Export workspace as ZIP".
  ...
  // Two interlocking links — used for "Copy as path".
  link: '<path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/>',
  // Database/duck icon for the DuckDB palette command.
  database: '<ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M3 5V19A9 3 0 0 0 21 19V5"/><path d="M3 12A9 3 0 0 0 21 12"/>',
};

const wrap = (body: string, size: number): string =>
...
export const PG_ICONS = {
  has: (name: string): boolean => name in ICONS,
  svgString: (name: string, size = 16): string => {
  ...
  mount: (el: Element | null, name: string, size?: number): void => {
    if (el) el.innerHTML = PG_ICONS.svgString(name, size);
  },
};

// Side-effect for legacy script-tag inclusion (sidepanel.html still <script
// src="lucide.js"> — pre-bundle). Re-exposes the registry on globalThis.
if (typeof globalThis !== 'undefined') {
  (globalThis as any).PG_ICONS = PG_ICONS;
}
